...
AddEventHandler('esx_unicornjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'unicorn' then
		print(('esx_unicornjob: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
RegisterServerEvent('esx_unicornjob:handcuff2')
AddEventHandler('esx_unicornjob:handcuff2', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx_unicornjob:handcuff', target)
end)
